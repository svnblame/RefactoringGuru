...
/**
 * Command is a behavioral design pattern that converts requests or simple
 * operations into objects.
 * 
 * The conversion allows deferred or remote execution of commands, storing command
 * history, etc.
 * 
 * In this example, the Command pattern is used to queue web scraping calls to the IMDB website
 * and execute them one by one. The queue itself is kept in a database that helps to preserve
...
class IMDBMovieScrapingCommand extends WebScrapingCommand
{
    ...
    public function parse(string $html): void
    {
        print_r($html); die(__METHOD__ . " " . __LINE__);
 ...
 * solutions available for use in real apps.
 */
class Queue 
{
    private $db;
